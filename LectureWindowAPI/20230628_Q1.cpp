...
void DrawGrid(HDC hdc, POINT start, POINT end, int n)
{
	double x_distance = abs(start.x - end.x) / (double)(n + 1);
	double y_distance = abs(start.y - end.y) / (double)(n + 1);

	for (int i = 0; i < n + 2; i++)
	{
		MoveToEx(hdc, start.x + x_distance * i, start.y , NULL);
		LineTo(hdc, start.x + x_distance * i, end.y);

		MoveToEx(hdc, start.x, start.y + y_distance * i, NULL);
		LineTo(hdc, end.x , start.y + y_distance * i);
	}
}

void DrawGridPoint(HDC hdc, POINT center, double width, double height, int n)
{
	double x_start = center.x - width / 2;
	double x_end = center.x + width / 2;
	double y_start = center.y - height / 2;
	double y_end = center.y + height / 2;

	double x_distance = width / (double)(n+1);
	double y_distance = height / (double)(n+1);

	for (int i = 0; i < n + 2; i++)
	{
		MoveToEx(hdc, x_start + x_distance * i, y_start, NULL);
		LineTo(hdc, x_start + x_distance * i, y_end);

		MoveToEx(hdc, x_start, y_start + y_distance * i, NULL);
		LineTo(hdc, x_end, y_start + y_distance * i);
	}
}
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hbrBackground = (HBRUSH)CreateSolidBrush(RGB(0, 60, 0));//GetStockObject(BLACK_BRUSH);//(COLOR_WINDOW+1);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		DrawGrid(hdc, { 50, 50 }, { 300, 300 }, 5);

		DrawGridPoint(hdc, { 500, 180 }, 250, 250, 5);
		EndPaint(hWnd, &ps);
	}
	break;
	case WM_DESTROY:
