...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN: //-> 가상 키 값 : wParam
	{
		...
		if (wParam == VK_RIGHT)
		{
			SetTimer(hWnd, timer_ID_1, 700, NULL);
			ptCurPos.x += circleR;
			if (ptCurPos.x + circleR > rectView.right) ptCurPos.x = rectView.right - circleR;
		}
		if (wParam == VK_LEFT)
		{
			ptCurPos.x -= circleR;
			if (ptCurPos.x - circleR < rectView.left) ptCurPos.x = rectView.left + circleR;

		}
		if (wParam == VK_UP)
		{
			ptCurPos.y -= circleR;
			if (ptCurPos.y - circleR < rectView.top) ptCurPos.y = rectView.top + circleR;

		}
		if (wParam == VK_DOWN)
		{
			ptCurPos.y += circleR;
			if (ptCurPos.y + circleR > rectView.bottom) ptCurPos.y = rectView.bottom - circleR;
	...
	case WM_TIMER:
	{
		...
		{
			ptCurPos.x += 20;
			if (ptCurPos.x + 20 > rectView.right) ptCurPos.x -= 20;
			InvalidateRect(hWnd, NULL, TRUE);
		}
	}
	break;
